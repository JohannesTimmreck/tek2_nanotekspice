...
C4030::C4030(std::string name) : Circuit(name)
{
    // set pins
    for (int i = 0; i <= 13; ++i)
    ...
    // set components
    _components.push_back(createComponent("xor", ""));
    _components.push_back(createComponent("xor", ""));
    _components.push_back(createComponent("xor", ""));
    _components.push_back(createComponent("xor", ""));
...
nts::Tristate C4030::compute(size_t pin)
{
    switch (pin) {
        case 1:
        case 2:
...
{
    (void)value;
    std::shared_ptr<IComponent> comp = std::make_shared<C4030>("");
