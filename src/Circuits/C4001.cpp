...
C4001::C4001(std::string name) : Circuit(name)
{
    // set pins
    for (int i = 0; i <= 13; ++i)
    ...
    // set components
    _components.push_back(createComponent("nor", ""));
    _components.push_back(createComponent("nor", ""));
    _components.push_back(createComponent("nor", ""));
    _components.push_back(createComponent("nor", ""));
...
nts::Tristate C4001::compute(size_t pin)
{
    switch (pin) {
        case 1:
        case 2:
...
{
    (void)value;
    std::shared_ptr<IComponent> comp = std::make_shared<C4001>("");
